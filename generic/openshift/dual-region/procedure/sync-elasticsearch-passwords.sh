...
# in both regions:
#   - elasticsearch-es-password-region-0: password from region 1's Elasticsearch
#   - elasticsearch-es-password-region-1: password from region 2's Elasticsearch
#
...
PASS_0=$(oc --context "$CLUSTER_1_NAME" -n "$CAMUNDA_NAMESPACE_1" get secret elasticsearch-es-elastic-user \
    -o jsonpath='{.data.elastic}' | base64 -d)

PASS_1=$(oc --context "$CLUSTER_2_NAME" -n "$CAMUNDA_NAMESPACE_2" get secret elasticsearch-es-elastic-user \
    -o jsonpath='{.data.elastic}' | base64 -d)

if [ -z "$PASS_0" ]; then
    echo "Error: Could not read elasticsearch-es-elastic-user from region 1 ($CLUSTER_1_NAME / $CAMUNDA_NAMESPACE_1)"
    exit 1
fi

if [ -z "$PASS_1" ]; then
    echo "Error: Could not read elasticsearch-es-elastic-user from region 2 ($CLUSTER_2_NAME / $CAMUNDA_NAMESPACE_2)"
    exit 1
fi
...
create_password_secret "$CLUSTER_1_NAME" "$CAMUNDA_NAMESPACE_1" "elasticsearch-es-password-region-0" "$PASS_0"
create_password_secret "$CLUSTER_2_NAME" "$CAMUNDA_NAMESPACE_2" "elasticsearch-es-password-region-0" "$PASS_0"

# Region 1 password → both regions
create_password_secret "$CLUSTER_1_NAME" "$CAMUNDA_NAMESPACE_1" "elasticsearch-es-password-region-1" "$PASS_1"
create_password_secret "$CLUSTER_2_NAME" "$CAMUNDA_NAMESPACE_2" "elasticsearch-es-password-region-1" "$PASS_1"

echo "Done. Cross-region password secrets synchronized:"
echo "  - elasticsearch-es-password-region-0 (from $CAMUNDA_NAMESPACE_1)"
echo "  - elasticsearch-es-password-region-1 (from $CAMUNDA_NAMESPACE_2)"
