...
# Script to create Keycloak realm Secret with templated realm configuration
# Usage: ./03-keycloak-create-realm-secret.sh [namespace]

NAMESPACE=${1:-camunda}

echo "Creating Keycloak realm Secret in namespace: $NAMESPACE"
...
if kubectl get secret keycloak-realm-config -n "$NAMESPACE" >/dev/null 2>&1; then
    echo "Deleting existing Secret keycloak-realm-config..."
    kubectl delete secret keycloak-realm-config -n "$NAMESPACE"
fi

# Create the Secret with the processed realm file
echo "Creating Secret keycloak-realm-config..."
kubectl create secret generic keycloak-realm-config -n "$NAMESPACE" \
    --from-file=realm-camunda-platform.json="$TEMP_REALM"

# Clean up temp file
rm "$TEMP_REALM"

echo "Secret 'keycloak-realm-config' created successfully!"
echo "The Secret contains the fully templated realm configuration."
