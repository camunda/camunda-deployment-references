...
if [[ "${MIGRATE_KEYCLOAK}" == "true" ]]; then
    ...
    # generate a Helm override so Identity can authenticate with Keycloak.
...
if [[ "${MIGRATE_KEYCLOAK}" == "true" ]]; then
    ...
    # Apply admin user override (generated by sync_keycloak_admin_credentials)
    if [[ -n "${KEYCLOAK_ADMIN_OVERRIDE_FILE:-}" && -f "${KEYCLOAK_ADMIN_OVERRIDE_FILE}" ]]; then
        HELM_VALUES_ARGS+=("${KEYCLOAK_ADMIN_OVERRIDE_FILE}")
    fi
fi
