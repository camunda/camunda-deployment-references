#!/bin/bash
# =============================================================================
# Identity Migration - Step 6: Validate and Show Cleanup Commands
# =============================================================================
...
echo "============================================================================="
echo "  Identity Migration - Step 6: Validate and Cleanup"
echo "============================================================================="
echo ""
...
kubectl run identity-db-test-${RANDOM} \
    --image=postgres:15 \
    --restart=Never \
    --rm -i \
    --namespace="${NAMESPACE}" \
    --env="PGPASSWORD=$(kubectl get secret "${DB_SECRET_NAME}" -n "${NAMESPACE}" -o jsonpath='{.data.password}' | base64 -d)" \
    -- psql -h "${TARGET_PG_HOST}" -p "${TARGET_PG_PORT:-5432}" -U "${TARGET_PG_USER:-identity}" -d "${TARGET_PG_DATABASE:-identity}" -c "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public'" 2>/dev/null && \
...
echo ""
echo "# ============================================================================="
echo "# DELETE BITNAMI IDENTITY POSTGRESQL DATA - THIS IS IRREVERSIBLE!"
echo "# ============================================================================="
echo ""
...
echo "kubectl delete pods -l app.kubernetes.io/name=postgresql,app.kubernetes.io/instance=${RELEASE_NAME} -n ${NAMESPACE}"
echo ""
...
echo "sleep 10"
echo "kubectl delete pvc -l app.kubernetes.io/name=postgresql,app.kubernetes.io/instance=${RELEASE_NAME} -n ${NAMESPACE}"
echo ""
...
echo "kubectl delete jobs -l app=identity-migration -n ${NAMESPACE}"
echo ""
