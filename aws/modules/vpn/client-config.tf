...
resource "null_resource" "download_certs" {
  provisioner "local-exec" {
    command = <<EOT
mkdir -p ./certs
aws s3 cp s3://${var.s3_bucket_name}/${local.ca_public_key_object_key} ./certs/ca-cert.pem
%{for client in var.client_key_names~}
aws s3 cp s3://${var.s3_bucket_name}/${local.client_keys[client].private_key_object_key} ./certs/${client}.crt
aws s3 cp s3://${var.s3_bucket_name}/${local.client_keys[client].public_key_object_key} ./certs/${client}.key
%{endfor~}
EOT
  }
...
data "local_file" "ca_cert_downloaded" {
  filename   = "${path.root}/certs/ca-cert.pem"
  depends_on = [null_resource.wait_for_cert_download_client]
}

data "local_file" "client_cert" {
  for_each   = toset(var.client_key_names)
  filename   = "${path.root}/certs/${each.key}.crt"
  depends_on = [null_resource.wait_for_cert_download_client]
}

data "local_file" "client_key" {
  for_each   = toset(var.client_key_names)
  filename   = "${path.root}/certs/${each.key}.key"
  depends_on = [null_resource.wait_for_cert_download_client]
}
...
resource "local_file" "vpn_config" {
  ...
  filename = "${path.root}/client-configs/${each.key}.ovpn"
...
<ca>
${data.local_file.ca_cert_downloaded.content}
</ca>
