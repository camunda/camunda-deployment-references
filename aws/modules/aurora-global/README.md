...
| <a name="output_jdbc_extra_url_parameters"></a> [jdbc\_extra\_url\_parameters](#output\_jdbc\_extra\_url\_parameters) | The extra\_url\_parameters rendered as a query-string fragment, already '&'-prefixed and key-sorted (e.g. '&failoverTimeoutMs=60000'). Append last when composing the URL, so caller parameters follow the module-owned ones. |
