...
| <a name="input_restore_container_command"></a> [restore\_container\_command](#input\_restore\_container\_command) | Command for the restore init container (Docker CMD). If empty, uses the image default. | `list(string)` | <pre>[<br/>  "bash",<br/>  "-c",<br/>  "export ZEEBE_BROKER_NETWORK_HOST=$(hostname -I | awk '{print $2}'); /usr/local/camunda/bin/camunda"<br/>]</pre> | no |
