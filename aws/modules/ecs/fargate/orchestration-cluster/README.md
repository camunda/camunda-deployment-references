...
| <a name="input_restore_container_entrypoint"></a> [restore\_container\_entrypoint](#input\_restore\_container\_entrypoint) | Entrypoint for the restore init container (Docker ENTRYPOINT). If empty, uses the image default. | `list(string)` | <pre>[<br/>  "bash",<br/>  "-c",<br/>  "/usr/local/camunda/bin/restore --backupId=$BACKUP_ID"<br/>]</pre> | no |
