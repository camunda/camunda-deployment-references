...
run "alb_listener_rule_created_by_default" {
  command = plan

  variables {
    # Default is true, but we still need the listener ARN since the rule resource references it.
  ...
  assert {
    ...
    error_message = "ALB webapp listener rule should be created by default (enable_alb_http_webapp_listener_rule defaults to true)"
  }
}

run "alb_listener_rule_skipped_when_disabled" {
  command = plan
  ...
  assert {
    ...
    error_message = "ALB webapp listener rule should be empty when enable_alb_http_webapp_listener_rule = false"
  }
}
...
run "prefix_used_in_resource_names" {
  ...
  assert {
    condition     = strcontains(aws_ecs_task_definition.connectors.family, "test-conn")
