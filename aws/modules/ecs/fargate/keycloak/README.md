# Keycloak (ECS Fargate) module

Deploys Camunda's Keycloak distribution as an ECS Fargate task/service in the AWS
single-region reference architecture. Modeled on the `management-identity` module.
...
| [aws_iam_policy.keycloak_logs_policy](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_policy) | resource |
...
| [aws_iam_role_policy_attachment.keycloak_logs_policy_attachment](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy_attachment) | resource |
...
| <a name="input_enable_alb_http_webapp_listener_rule"></a> [enable\_alb\_http\_webapp\_listener\_rule](#input\_enable\_alb\_http\_webapp\_listener\_rule) | Whether to create the ALB target group + listener rule for Keycloak (opt-in; requires a decided context path). Must be a known boolean at plan time. | `bool` | `false` | no |
| <a name="input_enable_realm_import"></a> [enable\_realm\_import](#input\_enable\_realm\_import) | When true, Keycloak imports a realm on startup: the container writes the JSON provided via the KEYCLOAK\_REALM\_IMPORT\_JSON environment variable (typically injected from a Secrets Manager secret) to the import directory and starts with `--import-realm`. Import is skipped for realms that already exist. Requires a secret named KEYCLOAK\_REALM\_IMPORT\_JSON in var.secrets. | `bool` | `false` | no |
...
| <a name="input_image"></a> [image](#input\_image) | The container image to use for Keycloak | `string` | `"camunda/keycloak:quay-26.6.4"` | no |
...
| <a name="input_service_connect_dns_name"></a> [service\_connect\_dns\_name](#input\_service\_connect\_dns\_name) | Service Connect discovery/DNS name registered for Keycloak, reachable in-cluster at http://<this>:18080/auth. Exposed as an input so callers can build backend URLs from a constant instead of routing this name through the resource graph. | `string` | `"keycloak"` | no |
...
| Name | Description |
| ---- | ----------- |
| <a name="output_keycloak_service_connect"></a> [keycloak\_service\_connect](#output\_keycloak\_service\_connect) | Service Connect DNS name for Keycloak (reachable at http://<this>:18080/auth within the ECS cluster). |
<!-- END_TF_DOCS -->
