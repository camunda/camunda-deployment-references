...
| <a name="input_rdbms_jdbc_url"></a> [rdbms\_jdbc\_url](#input\_rdbms\_jdbc\_url) | Override for the RDBMS secondary-storage JDBC URL. When null (default), the URL is taken from the infra layer's aurora\_jdbc\_url output. Set it to point this app layer at a database provisioned outside this reference architecture, or at an infra state that predates the aurora\_jdbc\_url output. Only used when secondary storage is 'rdbms'. | `string` | `null` | no |
