...
resource "aws_ecs_task_definition" "db_seed" {
  ...
  execution_role_arn       = local.infra.region_0_ecs_task_execution_role_arn
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        { name = "AURORA_ENDPOINT", value = local.infra.aurora_primary_cluster_endpoint },
      ...
      secrets = [
        {
          ...
          valueFrom = local.infra.db_admin_secret_arn
        }
      ]
...
resource "null_resource" "run_db_seed_task" {
  ...
  triggers = {
    aurora_endpoint = local.infra.aurora_primary_cluster_endpoint
    db_name         = var.db_name
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      NETWORK_CONF='{"awsvpcConfiguration":{"subnets":${jsonencode(local.infra.region_0_private_subnets)},"securityGroups":${jsonencode([local.infra.region_0_camunda_ports_sg_id, local.infra.region_0_package_80_443_sg_id])},"assignPublicIp":"DISABLED"}}'

      echo "Running one-time DB seed task..."
      TASK_ARN=$(aws ecs run-task \
        --region "${data.aws_region.region_0.id}" \
        --cluster "${local.infra.region_0_ecs_cluster_arn}" \
        --launch-type FARGATE \
  ...
  depends_on = [aws_ecs_task_definition.db_seed]
}
