...
| <a name="input_db_extra_wrapper_plugins"></a> [db\_extra\_wrapper\_plugins](#input\_db\_extra\_wrapper\_plugins) | Additional AWS Advanced JDBC Wrapper plugins to append to the generated JDBC URL. 'failover' (and 'iam' when db\_iam\_auth\_enabled) are always set, so list only the extras, e.g. ['efm2']. Only applies when secondary\_storage\_type = 'rdbms'. | `list(string)` | `[]` | no |
