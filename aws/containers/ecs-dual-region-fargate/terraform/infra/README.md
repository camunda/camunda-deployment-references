...
| <a name="input_db_failover_timeout_ms"></a> [db\_failover\_timeout\_ms](#input\_db\_failover\_timeout\_ms) | Maximum time in milliseconds the AWS JDBC Wrapper keeps trying to reconnect after a cluster failover (failoverTimeoutMs). Only applies when secondary\_storage\_type = 'rdbms'. | `number` | `60000` | no |
