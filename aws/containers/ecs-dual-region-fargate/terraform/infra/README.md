...
| <a name="input_db_extra_url_parameters"></a> [db\_extra\_url\_parameters](#input\_db\_extra\_url\_parameters) | Additional query parameters appended to the generated JDBC URL, e.g. { connectTimeout = "5000" }. Entries here override the reference architecture's own (failoverTimeoutMs = 60000). The Aurora module rejects the parameters it builds itself (wrapperPlugins, globalClusterInstanceHostPatterns, TLS mode) as well as keys or values containing '&' or '='. Only applies when secondary\_storage\_type = 'rdbms'. | `map(string)` | `{}` | no |
| <a name="input_db_extra_wrapper_plugins"></a> [db\_extra\_wrapper\_plugins](#input\_db\_extra\_wrapper\_plugins) | Additional AWS Advanced JDBC Wrapper plugins to append to the generated JDBC URL. The module always sets 'failover' (and 'iam' when db\_iam\_auth\_enabled), so list only the extras, e.g. ['readWriteSplitting']. Note that EFM/EFM2 also need 'initialConnection' on the Aurora Global writer endpoint this deployment connects to, so adding 'efm2' alone will not attach it. Only applies when secondary\_storage\_type = 'rdbms'. | `list(string)` | `[]` | no |
