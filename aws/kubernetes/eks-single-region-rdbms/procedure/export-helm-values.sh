...
# RDBMS secondary storage (orchestration) database
export DB_ORCHESTRATION_NAME="$(terraform console <<<local.camunda_database_orchestration | tail -n 1 | jq -r)"
export DB_ORCHESTRATION_USERNAME="$(terraform console <<<local.camunda_orchestration_db_username | tail -n 1 | jq -r)"
export DB_ORCHESTRATION_PASSWORD="$(terraform output -raw camunda_orchestration_db_password)"
