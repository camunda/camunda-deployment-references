locals {
  aurora_cluster_name = "cluster-name-pg-rdbms" # Replace "cluster-name" with your cluster's name
  ...
  # RDBMS secondary storage: dedicated database for the Orchestration cluster
  # (Operate/Tasklist/Zeebe exporter) used instead of Elasticsearch/OpenSearch.
  ...
  # RDBMS secondary storage: dedicated user for the orchestration DB
...
# To retrieve passwords after apply: terraform output -json | jq '.aurora_master_password.value, .camunda_identity_db_password.value, .camunda_webmodeler_db_password.value, .camunda_orchestration_db_password.value'
...
module "postgresql" {
  ...
  num_instances  = "1" # only one instance, you can add other read-only instances if you want
...
output "camunda_orchestration_db_password" {
  description = "Orchestration (RDBMS secondary storage) DB password"
