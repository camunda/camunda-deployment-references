...
bare_ids="$(jq -r '[.brokers[] | select(.brokerId | test("_[0-9]+$") | not) | .brokerId] | join(", ")' \
    "$OUTPUT_FILE")"
if [ -n "$bare_ids" ]; then
    fail "brokers [$bare_ids] report a plain node ID, so this cluster is not zone-aware; this architecture deploys orchestration.partitionDistribution.scheme=zoned"
    echo
