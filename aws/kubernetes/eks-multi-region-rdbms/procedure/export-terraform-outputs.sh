...
CAMUNDA_REGION_SLOTS="$(_tf_get '.region_slot_count.value')"
export CAMUNDA_REGION_SLOTS
...
TF_ACTIVE_REGION_COUNT="$(_tf_get '.active_region_count.value')"
export TF_ACTIVE_REGION_COUNT
...
AWS_REGIONS="$(_tf_get '[.regions.value | to_entries | sort_by(.key | tonumber) | .[].value.region] | join(" ")')"
export AWS_REGIONS
SUBMARINER_CLUSTER_IDS="$(_tf_get '[.regions.value | to_entries | sort_by(.key | tonumber) | .[].value.short_name] | join(" ")')"
export SUBMARINER_CLUSTER_IDS
...
CLUSTER_CONTEXTS="$(_tf_get '[.regions.value | to_entries | sort_by(.key | tonumber) | .[].value.short_name | "cluster-" + .] | join(" ")')"
export CLUSTER_CONTEXTS
REGION_VPC_CIDRS="$(_tf_get '[.vpc_cidr_blocks.value | to_entries | sort_by(.key | tonumber) | .[].value] | join(" ")')"
export REGION_VPC_CIDRS
REGION_SERVICE_CIDRS="$(_tf_get '[.service_cidr_blocks.value | to_entries | sort_by(.key | tonumber) | .[].value] | join(" ")')"
export REGION_SERVICE_CIDRS
...
CAMUNDA_ZONE_NAMES="$(_tf_get '.zone_names.value | join(" ")')"
export CAMUNDA_ZONE_NAMES
EKS_CLUSTER_NAMES="$(_tf_get '[.cluster_names.value | to_entries | sort_by(.key | tonumber) | .[].value] | join(" ")')"
export EKS_CLUSTER_NAMES

CAMUNDA_RDBMS_URL="$(_tf_get '.camunda_rdbms_url.value // empty')"
export CAMUNDA_RDBMS_URL
CAMUNDA_RDBMS_USERNAME="$(_tf_get '.database_username.value // empty')"
export CAMUNDA_RDBMS_USERNAME
CAMUNDA_RDBMS_PASSWORD="$(_tf_get '.database_password.value // empty')"
export CAMUNDA_RDBMS_PASSWORD
AURORA_GLOBAL_CLUSTER_ID="$(_tf_get '.database_global_cluster_id.value // empty')"
export AURORA_GLOBAL_CLUSTER_ID
