...
// errors, on a 409 and on transient 5xx responses. While partitions are being
// redistributed the gateway can briefly reject a mutating request with a 500 (the
// change is not applied), so a single-shot call is racy and can fail the whole
// failover/failback step on an intermittent server error.
//
// A 409 is the same class of transient rejection: the cluster actuator maps a
// concurrent modification to it, meaning another configuration change targeting
// an overlapping scope is still in flight. Exporter pause/resume is itself such a
// change and is applied asynchronously, so the very next scaling PATCH can land
// while it is still running — the caller has nothing to assert on, it just has to
// come back later.
//
// This retries with a backoff until the request is accepted (202), a definitive
// non-transient status is returned — which the caller then asserts on — or the
// retry budget is exhausted. It returns the last status, body and error observed.
func GatewayManagementMutate(t *testing.T, kubectlOptions *k8s.KubectlOptions, method, path string, payload []byte, maxRetries int, backoff time.Duration) (int, string, error) {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err == nil && status < 500 && status != http.StatusConflict {
