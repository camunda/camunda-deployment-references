...
# Patch setup-db-secret with Keycloak database credentials (CI/Test only - for embedded Keycloak)
# This script adds the Keycloak DB credentials to an existing setup-db-secret
...
kubectl patch secret setup-db-secret --namespace "$CAMUNDA_NAMESPACE" --type='json' -p='[
  ...
  {"op": "add", "path": "/data/DB_KEYCLOAK_PASSWORD", "value": "'"$(echo -n "$DB_KEYCLOAK_PASSWORD" | base64)"'"}
]'
