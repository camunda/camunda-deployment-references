...
Execute these steps in order. All commands are **bash**. If the user's shell is fish, wrap commands in `bash -c '...'` or write the script to `/tmp/` and run with `bash /tmp/gen-sa-kubeconfig.sh`.
...
```bash
SSO_ROLE="arn:aws:iam::444804106854:role/aws-reserved/sso.amazonaws.com/eu-central-1/AWSReservedSSO_SystemAdministrator_555f3db864dcee7e"

for cluster_region in "rbc-bench-7ce10-us-east-1:us-east-1" "rbc-bench-7ce10-us-east-2:us-east-2"; do
  CLUSTER_NAME="${cluster_region%%:*}"
  REGION="${cluster_region##*:}"
  aws eks create-access-entry --cluster-name "$CLUSTER_NAME" --region "$REGION" --profile infex --principal-arn "$SSO_ROLE" 2>&1 || true
  aws eks associate-access-policy --cluster-name "$CLUSTER_NAME" --region "$REGION" --profile infex --principal-arn "$SSO_ROLE" \
    --policy-arn arn:aws:eks::aws:cluster-access-policy/AmazonEKSClusterAdminPolicy --access-scope type=cluster 2>&1 || true
done
```
...
Write the script below to `/tmp/gen-sa-kubeconfig.sh` and run it with `bash /tmp/gen-sa-kubeconfig.sh`:
...
Usage:
```bash
export KUBECONFIG=/tmp/kubeconfig-sa-rbc-bench-us-east-1.yml
kubectl get pods -n c8-snap-cluster-0
```

## Important notes

- All commands are bash. If the user's shell is fish, write the script to `/tmp/gen-sa-kubeconfig.sh` and run with `bash /tmp/gen-sa-kubeconfig.sh`.
