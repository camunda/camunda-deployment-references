...
echo "TF_CLI_ARGS_init=-backend-config='bucket=tf-state-multi-reg' -backend-config='key=state/$TF_PREFIX/terraform.tfstate' -backend-config='region=eu-central-1' -backend-config='encrypt=true'" >> "$GITHUB_ENV"
