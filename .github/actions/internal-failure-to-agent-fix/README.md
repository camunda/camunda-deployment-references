...
| `bot-handle` | <p>Handle mentioned to re-engage the coding agent when a new occurrence is grouped onto an existing issue. Defaults to "@copilot", the GitHub Copilot cloud agent's platform trigger handle — the only mention that re-starts a work session, regardless of the backing model. The bot login the agent commits under ("@Claude", "@codex") and the suggestedActors assignment slug ("anthropic-code-agent" etc.) do NOT re-trigger it.</p> | `false` | `@copilot` |
...
- uses: camunda/camunda-deployment-references/.github/actions/internal-failure-to-agent-fix@main
  with:
    ...
    bot-handle:
    # Handle mentioned to re-engage the coding agent when a new occurrence is grouped onto an existing issue. Defaults to "@copilot", the GitHub Copilot cloud agent's platform trigger handle — the only mention that re-starts a work session, regardless of the backing model. The bot login the agent commits under ("@Claude", "@codex") and the suggestedActors assignment slug ("anthropic-code-agent" etc.) do NOT re-trigger it.
    #
    # Required: false
    # Default: @copilot
```
