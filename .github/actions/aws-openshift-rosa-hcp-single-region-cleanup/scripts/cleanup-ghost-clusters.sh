...
# Description:
# This script deletes clusters that have no resources left in AWS but still appear in the OpenShift console.
...
echo "$raw_clusters" | jq -c '.[]' | while read -r cluster; do
  ...
  if ! AWS_REGION="$region_id" rosa delete oidc-provider --oidc-config-id "${oidc_config_id}" --yes --mode auto; then
    ...
    # Delete the OIDC provider directly via AWS IAM
