...
| `github-token` | <p>Token used to read and update the shared PR comment. Defaults to the workflow-provided GITHUB<em>TOKEN. The token needs <code>pull-requests: write</code> permission for the comment to be posted. Beware that a job-level <code>permissions:</code> block REPLACES the workflow-level one, so it must repeat every permission the job needs. Fork pull requests get a read-only GITHUB</em>TOKEN whatever the workflow requests. When the comment cannot be written the action emits an <code>::error::</code> annotation and skips it; it never fails the job.</p> | `false` | `${{ github.token }}` |
...
- uses: camunda/camunda-deployment-references/.github/actions/internal-helm-deprecation-check@main
  with:
    ...
    # needs. Fork pull requests get a read-only GITHUB_TOKEN whatever
    # the workflow requests. When the comment cannot be written the
    # action emits an `::error::` annotation and skips it; it never
    # fails the job.
    #
    # Required: false
