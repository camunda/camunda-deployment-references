...
| `github-token` | <p>Token used to read and update the shared PR comment. Defaults to the workflow-provided GITHUB_TOKEN. The token needs <code>pull-requests: write</code> permission for the comment to be posted. Beware that a job-level <code>permissions:</code> block REPLACES the workflow-level one, so it must repeat every permission the job needs. When the permission is missing the action emits an <code>::error::</code> annotation and skips the comment; it never fails the job.</p> | `false` | `${{ github.token }}` |
...
- uses: camunda/camunda-deployment-references/.github/actions/internal-helm-deprecation-check@main
  with:
    ...
    # needs. When the permission is missing the action emits an
    # `::error::` annotation and skips the comment; it never fails
    # the job.
    #
    # Required: false
